...
evch_results = pd.DataFrame(columns = ['id', 'e_dem', 'e_ch', 'satisfaction', 'cp'])
...
while(current_datetime <= end_datetime):
        ...
        if(str(current_datetime.time()) == ev_arrivals.loc[i, 't_dep']):
            for j in range(n_cs):
                if(ev_id[j] == ev_arrivals.loc[i, 'id']):
                    ...
                    evch_results = evch_results._append(ev_ch_results, ignore_index = True)
                    ev_id[j] = ''
    ...
    ev_pwr = FCI_ChSt.wfa(ev_dem, ev_ch, ev_tin, ev_tout, p_load, p_res, current_datetime.timestamp(), 1.0)
    ...
    if (current_datetime.month != last_datetime.month):
        ...
        pwr_results.to_csv(f'main/scenarios/results/bs/pwr/bs_pwr_{last_datetime.month}.csv', index=False)
        evch_results.to_csv(f'main/scenarios/results/bs/ch/bs_evch_{last_datetime.month}.csv', index=False)
